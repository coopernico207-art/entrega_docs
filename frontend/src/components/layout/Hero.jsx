...
export default function Hero() {
  ...
  // Carga instantánea síncrona (0ms) sin depender del backend para arrancar
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/configuracion-web')
      ...
      .catch(() => {
        // Fallback silencioso: no interrumpe la navegación pública
      });
  }, []);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center w-full">
                  ...
                  <div className="lg:col-span-8 space-y-3 sm:space-y-5 text-center lg:text-left">
                    ...
                    <h1 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-white tracking-tight leading-tight">
                      {(configWeb && configWeb.hero_title) || slide.titulo} <br />
                      <span className="text-amber-300 drop-shadow-md">{(configWeb && configWeb.hero_sub) || slide.subtitulo}</span>
                    </h1>

                    <p className="text-xs sm:text-base text-gray-200 font-normal max-w-2xl mx-auto lg:mx-0 leading-relaxed">
                      {(configWeb && configWeb.hero_desc) || (
                        <>
