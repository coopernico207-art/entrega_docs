...
import { plantelData, obtenerConfigWebInicial, persistirConfigWebLocal } from '../../data/plantelData';
...
export default function VisionPlantel() {
  // Carga instantánea síncrona (0ms) sin depender de respuesta de red inicial
  const [configWeb, setConfigWeb] = useState(obtenerConfigWebInicial);

  useEffect(() => {
    // Sincronización silenciosa en segundo plano
    fetch('/api/admin/configuracion-web')
      .then(res => res.ok ? res.json() : null)
      .then(data => {
        if (data && data.configuracion) {
          setConfigWeb(prev => ({ ...prev, ...data.configuracion }));
          persistirConfigWebLocal(data.configuracion);
        }
      })
      .catch(() => {
        // Fallback silencioso: no bloquea ni afecta la vista pública si el backend está apagado
      });
  }, []);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:shadow-md transition-all space-y-3">
          ...
          <p className="text-xs text-gray-600 leading-relaxed">
            {(configWeb && configWeb.mision) || plantelData.mision}
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border-2 border-[#bc955c]/50 shadow-md hover:shadow-lg transition-all space-y-3 bg-gradient-to-b from-amber-50/30 to-white">
          ...
          <p className="text-xs text-gray-700 leading-relaxed font-medium">
            {(configWeb && configWeb.vision) || plantelData.vision}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs sm:text-sm">
          <div className="bg-gray-50 p-5 rounded-2xl border border-gray-200 space-y-3">
            ...
            <div className="space-y-2.5 text-gray-700">
              ...
              <p><strong>Modalidad:</strong> {plantelData.datosGenerales.modalidad}</p>
              <p><strong>Régimen:</strong> {plantelData.datosGenerales.regimen}</p>
